$(document).ready(function() {
  ...
  var pastCities = JSON.parse(localStorage.getItem("city-list"));
  if (pastCities) {
 ...
 function makeList(city) {
  ...
  function saveLocalStorage(newCity) {
    var pastCityList = JSON.parse(localStorage.getItem("city-list"));
    if (!pastCityList) {
      var cityList = [];
        ...
        for (var i = 0; i < results.length; i++) {
          if (results[i].dt_txt.indexOf("12:00:00") !== -1) {
            ...
            var card = $("<div>").addClass(
              "card col-md-2 ml-4 bg-info text-white"
            );
